...
| **Logo** (`public/images/novage-logo.svg`) | Run `npm run og-image` to regenerate `public/images/og-default.png`, and `npm run linkedin-images` to regenerate the LinkedIn company banner and square logo and the founder's profile banner in `brand/linkedin/` (then upload them); check the header and the `logo` in `src/data/organization.ts` |
